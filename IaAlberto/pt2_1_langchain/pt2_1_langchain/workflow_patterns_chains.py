...
================================================================
Aplicación: Generador de Recetas Personalizadas

Este patrón encadena múltiples llamadas LLM donde la salida de una
se convierte en la entrada de la siguiente:
1. Analizar ingredientes disponibles
2. Sugerir tipo de plato apropiado
3. Generar receta completa
4. Crear lista de compras con ingredientes faltantes
...
class TokenTracker:
    ...
    Attributes:
        ...
        reasoning_tokens: Tokens de razonamiento (si disponible)
        calls: Número de llamadas al modelo
    """
    input_tokens: int = 0
    output_tokens: int = 0
...
def step1_analyze_ingredients(ingredients: str, tracker: TokenTracker) -> str:
    """
    Paso 1: Analiza los ingredientes disponibles y los categoriza.
    """
    prompt = f"""Analiza los siguientes ingredientes y categorízalos:
...
def step2_suggest_dish(analysis: str, tracker: TokenTracker) -> str:
    """
    Paso 2: Sugiere un tipo de plato basado en el análisis.
    """
...
def step3_generate_recipe(dish_suggestion: str, original_ingredients: str, tracker: TokenTracker) -> str:
    """
    Paso 3: Genera la receta completa.
    """
...
def step4_shopping_list(recipe: str, available_ingredients: str, tracker: TokenTracker) -> str:
    """
    Paso 4: Crea lista de compras con ingredientes faltantes.
    """
...
def recipe_generator_chain(ingredients: str):
    """
    Pipeline principal de Prompt Chaining para generar recetas.
    
    Flujo:
    Ingredientes → Análisis → Sugerencia → Receta → Lista de compras
    
    Args:
        ingredients: Lista de ingredientes disponibles
        
    Returns:
        dict: Resultados de cada paso del pipeline
    """
...
def main():
    """Función principal para demostrar el patrón Prompt Chaining."""
    print("\n" + "=" * 60)
